...
def load_ply_corners(model_path, model_scale=1.0):
    ...
    能够正确处理带有法线、颜色等附加属性的二进制/ASCII PLY文件
    """
    ...
    with open(model_path, 'rb') as f:
        ...
        for line in header_lines:
            if line.startswith('element vertex'):
                ...
                # 寻找当前 element vertex 对应的属性
                # 我们需要检查之前是否遇到了 element vertex
                is_after_vertex = False
                for h in header_lines:
                    if h.startswith('element vertex'):
                        is_after_vertex = True
        ...
        if is_ascii:
            ...
            vertex_size = sum(type_sizes.get(t, 4) for t in vertex_properties)
            
            # 判断字节序
            is_big_endian = any('big_endian' in line.lower() for line in header_lines)
            endian = '>' if is_big_endian else '<'
            
            # x, y, z 通常是前三个 float属性
            for _ in range(vertex_count):
                data = f.read(vertex_size)
                if len(data) == vertex_size:
                    x, y, z = struct.unpack_from(endian + 'fff', data, 0)
...
def load_all_models(models_dir, model_scale=1.0):
    ...
    for filename in sorted(os.listdir(models_dir)):
        if filename.endswith('.ply'):
            try:
                # 尝试从文件名提取obj_id (e.g., obj_000001.ply -> 1)
                obj_id_str = filename.replace('obj_', '').replace('.ply', '')
                try:
                    obj_id = int(obj_id_str)
                except ValueError:
                    # 如果有 _sym 等后缀，我们提取前面的数字部分
                    if '_' in obj_id_str:
                        try:
                            obj_id = int(obj_id_str.split('_')[0])
                        except ValueError:
                ...
                # 如果这个obj_id已经被加载过了，有可能是原版和sym版重复，我们可以选择跳过或者覆盖
                if obj_id in models:
                    continue
